...
def compute_tvt_score(
    ...
    with torch.no_grad():
        ...
        inputs = F.interpolate(
            inputs,
            size=(net.teacher_img_size, net.teacher_img_size),
            mode='bilinear',
            align_corners=False)
        ...
        dsize = (max(feats_t.size(-2), feats_s.size(-2)),
                 max(feats_t.size(-1), feats_s.size(-1)))
        feats_t = F.interpolate(
            feats_t, dsize, mode='bilinear', align_corners=False)
        feats_s = F.interpolate(
            feats_s, dsize, mode='bilinear', align_corners=False)
        ...
        student_idx_grp = [1, 6]
        for i, (idx_t,
                idx_s) in enumerate(zip(teacher_idx_grp, student_idx_grp)):
            feat_t_temp = feats_t_group[idx_t]
            feat_s_temp = feats_s_group[idx_s]
            dsize = (max(feat_t_temp.size(-2), feat_s_temp.size(-2)),
                     max(feat_t_temp.size(-1), feat_s_temp.size(-1)))
            feat_t_temp = F.interpolate(
                feat_t_temp, dsize, mode='bilinear', align_corners=False)
            feat_s_temp = F.interpolate(
                feat_s_temp, dsize, mode='bilinear', align_corners=False)
            ...
            outputs.append(
                inter_distill_loss(
                    feat_s_temp, feat_t_temp, transform_type='attention'))
