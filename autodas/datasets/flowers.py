...
class Flowers(BaseDataset):
    ...
    def __init__(self, data_path, split):
        ...
        assert os.path.exists(data_path), "Data path '{}' not found".format(
            data_path)
        splits = ['train', 'test']
        assert split in splits, "Split '{}' not supported for Flowers".format(
            split)
        self.data_path = data_path
        self.labels = loadmat(os.path.join(data_path,
                                           'imagelabels.mat'))['labels'][0] - 1
        all_files = loadmat(os.path.join(data_path, 'setid.mat'))
        if split == 'train':
            self.ids = np.concatenate(
                [all_files['trnid'][0], all_files['valid'][0]])
        else:
