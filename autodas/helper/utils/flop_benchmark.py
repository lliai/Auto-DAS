...
def compute_average_flops_cost(model):
  ...
  """
    ...
    for module in model.modules():
        if isinstance(module, (torch.nn.Conv2d, torch.nn.Linear)) or hasattr(
                module, 'calculate_flop_self'):
...
def pool_flops_counter_hook(pool_module, inputs, output):
    ...
    overall_flops = batch_size * out_C * output_height * \
        output_width * kernel_size * kernel_size
...
def conv2d_flops_counter_hook(conv_module, inputs, output):
    ...
    out_channels = conv_module.out_channels
    groups = conv_module.groups
    conv_per_position_flops = kernel_height * \
        kernel_width * in_channels * out_channels / groups
