...
def evolution_search_jointly(img,
                             ...
                             sample_num=50,
                             seed: int = 42):
    """Evolution search algorithm for jointly.

    Args:
        img (torch.Tensor): [B, C, H, W]
        label (torch.Tensor): [B]
        iterations (int, optional): iterations. Defaults to 1000.
        logger ([type], optional): logger. Defaults to None.
        popu_size (int, optional): population size. Defaults to 50.
        prob_mutate (float, optional): probability of mutation.
            Defaults to 0.1.
        prob_crossover (float, optional): probability of crossover.
            Defaults to 0.5.
        topk (float, optional): topk. Defaults to 0.2.
        scale (bool, optional): scale two score list to [0, 1] and
            then add them. Defaults to False.
        api (NB101API, optional): api. Defaults to None.
        sample_num (int, optional): number of samples. Defaults to 50.
    ...
    scores = [
        compute_rank_consistency(img, label, s[0], s[1], scale, api,
                                 sample_num)[0] for s in population
    ]
    argidxs = np.argsort(scores)[::-1]
    running_struct = population[argidxs[0]]
    print(
    ...
    save_name = f'evolution_jointly_{type(newbie).__name__}_{iterations}_{popu_size}_{seed}'
...
if __name__ == '__main__':
    ...
    best_interaction, best_instinct, best_rk = evolution_search_jointly(
        ...
        args.prob_crossover, args.topk, args.scale, api, args.sample_num,
        args.seed)
