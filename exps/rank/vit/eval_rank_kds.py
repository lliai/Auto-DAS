...
def compute_diswot_procedure(img, api=None, sample_num: int = 50):
...
def compute_kd_procedure(img, api=None, sample_num=50, kd_loss='nst'):
    ...
    for i, (struct, acc) in enumerate(iter(api)):
        ...
        # using linear to make them match
        linear = nn.Linear(teacher_feat.size(1), student_feat.size(1))
        teacher_feat = linear(teacher_feat)
        student_feat = linear(student_feat)
...
if __name__ == '__main__':
    ...
    kd, sp, ps = compute_kd_procedure(img, api, args.sample_num, args.kd_loss)
